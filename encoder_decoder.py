...
def _clones(module: nn.Module, N: int):
    "Produce N identical layers."
    return nn.ModuleList([copy.deepcopy(module) for _ in range(N)])

def _subsequent_mask(size: int) -> torch.Tensor:
...
class EncoderDecoder(nn.Module):
    ...
    def __init__(self, encoder: nn.Module, decoder: nn.Module, src_embed, tgt_embed, generator: nn.Module):
        super().__init__()
...
class MultiHeadedAttention(nn.Module):
    ...
    def forward(self, query, key, value, mask = None):
        ...
        x, self_attn = MultiHeadedAttention.attention(query, key, value, mask=mask, dropout=self.dropout)
